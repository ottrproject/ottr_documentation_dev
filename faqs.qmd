...
::: {.callout-warning}
### Forked repositories and OTTRfy

OTTRfy does not work on repositories that were created as forks. If you need to OTTRfy existing content, start from a regular new repository instead of a forked repository.
:::

<div class="troubleshooting-divider" aria-hidden="true"></div>
...
-   Check that the GitHub repository `GH_PAT` is up to date. (An associated GitHub Actions error that suggests the `GH_PAT` needs to be updated is an `exit code 128`). Check out how to do this on the [cheatsheet](https://ottrproject.org/cheatsheets/ottr_token.html).
...
-   Your workflow files may be out of date compared to the template and you may need to request a sync by opening an issue in the template repository. See steps for this on the [cheatsheet](https://www.ottrproject.org/cheatsheets/ottr_sync.html).

- Push a blank commit/arbitrary change (like a blank space). Make the modification in the PR that the content is not rendering to re-run the workflow. 

- Check [GitHub Status](https://www.githubstatus.com/) if GitHub Actions failures look widespread or unusual. If GitHub Actions status is failing, wait until GitHub reports that it has recovered, then re-trigger your actions.

<div class="troubleshooting-divider" aria-hidden="true"></div>

### Spell check showing outdated results

If the spell check is showing old results, delete the old files in `check_reports/` and re-run the check by pushing a new commit to the pull request.
...
![](resources/screenshots/gha-actions-error.png)
...
| [Quarto Websites](getting_started.html#quarto-websites) ⭐ | New websites, starting fresh, existing `.qmd` website files | `.qmd` | GitHub Pages |
| [R Markdown Websites](getting_started.html#r-markdown-websites) | Existing `.Rmd` website files | `.Rmd` | GitHub Pages |
| [Dashboards](getting_started.html#dashboards) | Metrics tracking & reporting | `.Rmd` by default; `.qmd` if adapted to Quarto | GitHub Pages |
...
- Choose **Quarto** if you are starting a new project or already have `.qmd` files. It is the modern successor to R Markdown, supports R and Python, and is what OTTR recommends for all new content.
