...
<button class="btn copy-credit-snippet" type="button" data-copy-target="#all-course-credit-rows-code code">Copy rows</button>
<span class="copy-credit-status" aria-live="polite"></span>
...
<summary>
  <span class="choice-note">2. Johns Hopkins courses</span>
</summary>

These rows should be added to all _Johns Hopkins_ courses:

<button class="btn copy-credit-snippet" type="button" data-copy-target="#johns-hopkins-credit-rows-code code">Copy rows</button>
<span class="copy-credit-status" aria-live="polite"></span>

::: {#johns-hopkins-credit-rows-code}
```
...
<summary>
  <span class="choice-note">3. ITCR courses</span>
</summary>

Additionally, this row should be added for _all ITCR Courses_:

<button class="btn copy-credit-snippet" type="button" data-copy-target="#itcr-credit-rows-code code">Copy rows</button>
<span class="copy-credit-status" aria-live="polite"></span>

::: {#itcr-credit-rows-code}
```
...
<button id="copy-credit-role-table" class="btn" type="button">Copy table</button>
<span id="copy-credit-role-status" aria-live="polite"></span>
...
<script>
(function () {
  const tableButton = document.getElementById("copy-credit-role-table");
  const tableStatus = document.getElementById("copy-credit-role-status");
  const table = document.querySelector("#credit-role-table table");
  ...
  function tableToMarkdown(tableElement) {
    const headers = Array.from(tableElement.querySelectorAll("thead th")).map(cellText);
    const rows = Array.from(tableElement.querySelectorAll("tbody tr")).map((row) =>
      Array.from(row.querySelectorAll("td")).map(cellText)
    );
    const alignment = headers.map((header) => header === "Order" ? ":---:" : ":---");
    const markdownRows = [headers, alignment, ...rows];
  ...
  function showCopyStatus(statusElement, didCopy) {
    if (!statusElement) {
      return;
    }

    statusElement.textContent = didCopy
      ? " Copied!"
      : " Copy failed. Select and copy manually.";

    if (didCopy) {
      window.setTimeout(() => { statusElement.textContent = ""; }, 2000);
    }
  }

  if (tableButton && table) {
    tableButton.addEventListener("click", async () => {
      const didCopy = await copyText(tableToMarkdown(table));
      showCopyStatus(tableStatus, didCopy);
    });
  }

  document.querySelectorAll(".copy-credit-snippet").forEach((button) => {
