...
:::{.callout-warning}
The `cow` package may be deprecated in the future. For now, use `cow::borrow_chapter()`, but watch for deprecation warnings during rendering and update your course if a replacement helper is announced.
:::

To borrow a chapter from another course, create an `.qmd` or `.Rmd` as you normally would, with a `ottrpal::set_knitr_image_path()` in a chunk at the beginning of the file and a [`H1` title](https://www.markdownguide.org/basic-syntax/).
